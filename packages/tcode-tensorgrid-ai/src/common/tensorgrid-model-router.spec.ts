...
describe('routeTensorGridModel', () => {
    it('uses native providers only for matching family metadata', () => {
        ...
        expect(routeTensorGridModel(model('tg:auto:gpt-5.6', ['chat.completions']))).to.include({
            provider: 'openai',
            useResponseApi: true,
            responseApiFallbackToChat: false,
        });
        expect(routeTensorGridModel(model('tg:auto:gpt-5.6', ['responses', 'chat.completions']))).to.include({
            provider: 'openai',
            useResponseApi: true,
            responseApiFallbackToChat: false,
        });
